...
class MyDataset(Dataset):
    ...
    def _collect_annotations(self):
        ...
        for meta in self.config["META"]:
            meta_path, meta_type, meta_scale = meta["path"], meta.get("type", "default"), meta.get("scale", 1)
            meta_ext = os.path.splitext(meta_path)[-1]
            if meta_ext == ".json":
                with open(meta_path) as f:
                    meta_l = json.load(f)
                meta_l = meta_l * int(meta_scale)
            elif meta_ext == ".jsonl":
                meta_l = []
                with open(meta_path) as f:
            ...
            logger.info(f"{meta_path}, type {meta_type}: len {len(meta_l)}, sample scale: {meta_scale}")
            if "ratio" in meta:
                random.seed(0)
    ...
    def __getitem__(self, index):
        try:
            ...
            if isinstance(e, DataBriefReportException):
                ...
                logger.info(f"Item {index} errored, annotation:\n" f"{self.ann[index]}\n" f"Error:\n" f"{traceback.format_exc()}")
